...
/*
회원가입 조건(아이디는 몇 자 이상이라거나, 특수문자 포함 여부 등)은 보통 프론트에서 수행한다.
...
public class BoardValidator implements Validator {
	...
	*/
	...
	public boolean supports(Class<?> clazz) {
		...
		// 폼값을 받을때 사용하는 커맨드객체의 타입만 명시하면 된다. 
	...
	*/
	...
	public void validate(Object target, Errors errors) {
		...
		//폼값을 저장한 커맨드 객체가 검증에 적합한지 검사(필요한 경우에만 사용)
		...
		//만약 아이디가 null이거나 반값이라면 if문 내부 코드를 실행
		if(userid == null || userid.trim().isEmpty()) {
			// 직접 콘솔에 에러 내용 출력
			System.out.println("아이디를 입력해주세요.");
			/*
			폼값 검증시 오류가 있는 경우 처리형식
			-> Errors인스턴스.rejectValue(폼의 name속성, 에러객체명,
										디폴트메세지)
			*/
