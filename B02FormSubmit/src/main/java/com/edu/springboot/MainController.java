...
/*
JSP에서는 컨트롤러로 정의하기 위해
Servlet클래스를 정의한 후 매핑정보를 입력했다.
Spring Boot에서는 아래와 같이 @Controller 어노테이션을 부착하면 자동으로
컨트롤러 클래스로 정의된다.
또한 사용을 위해 별도의 인스턴스를 생성할 필요없이 스프링 컨에티너가 시작시
자동으로 자바빈(인스턴스)을 생성해준다.*/
@Controller
public class MainController {
	...
	"Perfix경로" + "메서드의 반환값" + "Suffix경로"에 해당하는 JSP파일을
	찾아 포워드한다. 
	 */
	...
	public String sub() {
		/*
		만약 views 하위에 별도의 홀더가 필요하다면 이와같이 슬러쉬로
		구분하여 작성하면 된다. sub폴더 하위의 sub.jsp를 찾아
		포워드한다. */
		return "sub/sub";
	}
	...
	public String registProcess(MemberDTO memberDTO, Model model) {
	    // model에 전달할 값 저장
