...
public class AddMember {
	...
	/*
	전파속성
	REQUIRED : 기존 트랙젝션에 의존한다. 즉 포함된 메서드나 포함시킨 메서드
		어느쪽이든 오류가 발생되면 모든 작업을 Rollback 처리한다.
	
	REQUIRES_NEW : 각각의 트랜젝션을 처리한다. 즉 포함시킨 메서드에서
		오류가 발생하더라도 포함된 메서드에서는 정상처리된다.
		포함된 메서드로 전파되지 않는다.
	*/
